...
function ekwa_slider_shortcode( $atts = [] ) {
	...
	ob_start();
	?>
	<div class="<?php echo esc_attr( implode( ' ', $wrapper_classes ) ); ?>" data-slides-count="<?php echo count( $slides ); ?>">
		...
		<?php if ( ! $is_mobile_banner && count( $slides ) > 1 ) : ?>
			<div class="ekwa-slider-controls">
				<button class="ekwa-slider-prev" aria-label="<?php esc_attr_e( 'Previous slide', 'ekwa-slider' ); ?>">&larr;</button>
				<button class="ekwa-slider-next" aria-label="<?php esc_attr_e( 'Next slide', 'ekwa-slider' ); ?>">&rarr;</button>
			</div>
